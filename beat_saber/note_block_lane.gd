...
func _on_current_beatmap_changed(current_beatmap: Variant):
	...
	for beatmap_object in current_beatmap._notes:
		...
		note_block_position += Vector3.RIGHT * note_block_line_width * beatmap_object._lineIndex
		note_block_position += Vector3.UP * note_block_line_height * beatmap_object._lineLayer
		...
		if beatmap_object._type == BeatmapObject.BeatmapObjectType.BOMB:
			...
			bomb_node.initialize(note_block_position, map_info, beatmap_object)
			beatmap_object_node = bomb_node
		else:
			var note_block_node: NoteBlock = note_block_scene.instantiate()
			note_block_node.initialize(note_block_position, map_info, beatmap_object)
