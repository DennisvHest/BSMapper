...
const LANE_HEIGHT := BEATMAP_OBJECT_LINE_SIZE * GRID_HEIGHT
...
func _position_object_type_selector() -> void:
	$ObjectTypeSelector.position = Vector3(
		-LANE_WIDTH / 2.0 - OBJECT_SELECTOR_MARGIN,
		GlobalSettings.player_height / 3.0 + LANE_HEIGHT / 2.0,
		OBJECT_SELECTOR_Z_OFFSET
	)

func _spawn_grid_cells() -> void:
	for line_index in GRID_WIDTH:
		for line_layer in GRID_HEIGHT:
			...
			cell.position += Vector3.RIGHT * BEATMAP_OBJECT_LINE_SIZE * line_index
			cell.position += Vector3.UP * BEATMAP_OBJECT_LINE_SIZE * line_layer

			# Center grid horizontally
			cell.position += Vector3.LEFT * LANE_WIDTH / 2
			cell.position += Vector3.RIGHT * BEATMAP_OBJECT_LINE_SIZE / 2

			# Center grid vertically
			cell.position += Vector3.UP * GlobalSettings.player_height * 1 / 3
			cell.position += Vector3.UP * BEATMAP_OBJECT_LINE_SIZE / 2
